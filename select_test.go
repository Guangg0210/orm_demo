...
func TestSelector_Having(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			name: "multiple",
			q: NewSelector[TestModel](db).GroupBy(C("Age")).
				Having(C("FirstName").EQ("Guan"), C("LastName").EQ("GangGang")),
			wantQuery: &Query{
				SQL:  "SELECT * FROM `test_model` GROUP BY `age` HAVING (`first_name` = ?) AND (`last_name` = ?);",
				Args: []any{"Guan", "GangGang"},
			},
		},
		{
			// 聚合函数
	...
	testCases := []struct {
		...
		},
		{
			...
			mockRows: func() *sqlmock.Rows {
				...
				res.AddRow([]byte("1"), []byte("Da"), []byte("18"), []byte("GangGang"), []byte("nothing"))
				return res
			}(),
		},
		{
			...
			mockRows: func() *sqlmock.Rows {
				...
				res.AddRow([]byte("1"), []byte("Da"), []byte("18"), []byte("GangGang"))
				return res
			}(),
			wantVal: &TestModel{
				Id:        1,
				FirstName: "Da",
				Age:       18,
				LastName:  &sql.NullString{String: "GangGang", Valid: true},
			},
		},
	}
...
//	goarch: arm64
//	pkg: orm_demo
//	BenchmarkQuerier_Get
//	BenchmarkQuerier_Get/unsafe
//	BenchmarkQuerier_Get/unsafe-8         	   10000	    148258 ns/op
//	BenchmarkQuerier_Get/reflect
//	BenchmarkQuerier_Get/reflect-8        	    4900	    215581 ns/op
//	PASS
...
func BenchmarkQuerier_Get(b *testing.B) {
	...
	_, err = db.sqlDB.Exec(TestModel{}.CreateSQL())
	if err != nil {
		b.Fatal(err)
	}

	res, err := db.sqlDB.Exec("INSERT INTO `test_model`(`id`,`first_name`,`age`,`last_name`)"+
		"VALUES (?,?,?,?)", 12, "Guan", 18, "GangGang")

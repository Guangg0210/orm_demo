...
func (s *sqlTestSuite) TestCRUD() {
	t := s.T()
	db, err := sql.Open("sqlite3", "file:test.sqlDB?cache=shared&mode=memory")
	if err != nil {
		t.Fatal(err)
	}
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
...
func TestSQLite(t *testing.T) {
	suite.Run(t, &sqlTestSuite{
		driver: "sqlite3",
		dsn:    "file:test.sqlDB?cache=shared&mode=memory",
	})
}
...
func memoryDBWithDB(db string, t *testing.T) *DB {
	orm, err := Open("sqlite3", fmt.Sprintf("file:%s.sqlDB?cache=shared&mode=memory", db))
	if err != nil {
		t.Fatal(err)
	}
	return orm
